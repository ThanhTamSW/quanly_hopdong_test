...
export function ImportContracts() {
  ...
  const handleDownloadTemplate = async () => {
    try {
      await importApi.downloadTemplate('contracts');
    } catch (error) {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      formData.append('type', 'contracts');
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await importApi.importContracts(formData);
      setImportResult({
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-600">
              Tải file template Excel để điền thông tin hợp đồng
            </p>
                  ...
                  <TableBody>
                    {previewData.slice(0, 5).map((row, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>{row.value.toLocaleString('vi-VN')} {row.currency}</TableCell>
                        <TableCell>{new Date(row.signDate).toLocaleDateString('vi-VN')}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
            ...
            <div className="space-y-4">
              ...
              >
                {importing ? (
                  ...
                  <>
                    <Upload className="h-4 w-4 mr-2" />
                    Import {validation.validRows} hợp đồng
                  </>
                )}
              </Button>
            </div>
